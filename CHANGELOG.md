...
- **Koreksi kuota VPC (`src/posts/2026-08-05-vpc-subnet-az-public-private-elastic-ip.md`):** kuota default ditulis 5 VPC per region (termasuk default VPC), dan request kenaikan kuota gratis. Sebelumnya tertulis 1 VPC dan ada tambahan biaya.
- **Secret Source Minggu 5 AWS re/Start (`src/posts/`):** 56 gambar resmi dipasang di 18 post 31 Agt sampai 4 Sep (EBS, EFS, S3, instance store, lab 183, CloudWatch, Glacier, Storage Gateway, Transfer Family/DataSync, lab 185, CloudWatch Logs/Events, lab 186, Organizations, tagging, lab 188, cost management, Trusted Advisor, configuration management, lab 187). Yang di-skip: overview lab 186 (dobel sama diagram per task), diagram lab 182 (belum ada post lab-nya), dan beberapa slide generik (cara akses Organizations, grafik console CloudWatch, how cloud storage works, use cases).
- **Secret Source Minggu 6 AWS re/Start (`src/posts/`):** 20 gambar resmi dipasang di 6 post 7 sampai 8 Sep (AMI building strategy dan launch template, IaC dan JSON/YAML, CloudFormation first stack dan lab 190, parameter, troubleshooting dan activity 191, rightsizing activity 189 plus Pricing Calculator before/after). Yang di-skip: panel credentials lab 189, topologi café intro (dobel), slide edit/design template dan TemplateURL S3.
...
- **[00:08 WIB] Polesan Desain Menyeluruh: Tipografi, Telemetri Asli, Potret Hover-Swap, Blog Index, dan Penataan Ulang About/Contact (`app.css`, `routes/+layout.svelte`, `app.html`, `lib/components/*`, `routes/(site)/blog/+page.svelte`, `lib/content/site.js`):**
  - *Tipografi*: enam keluarga font dipangkas menjadi tiga. Epilogue untuk judul, tombol, dan label; Plus Jakarta Sans untuk semua teks bacaan termasuk blog; Fira Code untuk label mono dan kode. Baskervville, Space Grotesk, Space Mono, dan Material Symbols (ikon tema kini SVG inline) tidak lagi diminta. Aturan `[class*='uppercase']` yang diam-diam menimpa `font-mono` dihapus, jadi label mono benar-benar tampil monospace. Lantai ukuran teks dinaikkan ke 11px: 61 pemakaian `text-[9px]` dan `text-[10px]` diganti token `text-label`.
  - *Telemetri jadi asli*: seam bar `SYS_ENGAGE`, `HUD_MATRIX_ONLINE`, `SYS_LATENCY`, `MEM_FOOTPRINT`, `NODE_VERIFIED`, `TTY_01`, `POD_042`, `PORTOFOLIO_OS v2.6.4`, dan `// VERIFIED` dihapus. Panel Engine Telemetry (tier, FPS, core, memori, dan jaringan yang terukur) pindah ke Contact dan mendapat stempel `Build YYYY-MM-DD` dari `__BUILD_TIME__` yang di-define di `vite.config.js`. Inspector Skills menampilkan hitungan nyata (jumlah teknologi dan layer), proficiency diberi label "self-assessed", dan blok output CLI palsu tidak dirender lagi (field `command` di `site.js` dibiarkan, tidak dipakai). Konten inspector desktop dan sheet mobile kini satu snippet.
  - *Potret*: komponen baru `PortraitSwap.svelte`. Gambar `primary` (ilustrasi) tampil default; `secondary` (foto formal) menyapu turun di belakang garis scan saat hover, fokus keyboard, atau tap di perangkat tanpa hover. Path diatur di `about.portrait` (`/portrait-illustrated.jpg` dan `/portrait-formal.jpg` di `static/`); bila file belum ada, komponen jatuh ke `SnakePlaceholder` yang kini menggambar dengan warna tema, bukan putih hardcoded.
  - *About dirampingkan*: prinsip kerja, baris "Currently", kartu availability, tombol CV, dan telemetri dipindah keluar. Tersisa potret, status, lokasi, intro, dua paragraf, dan statistik.
  - *Contact*: blok "Open for opportunities" dengan Type / Location / Roles, tombol View resume / CV, kartu Email dan Profiles dengan label yang jujur, panel telemetri, dan footer `© tahun nama` plus pill status.
  - *Blog index ditulis ulang*: app-shell desktop (panel scroll internal, wheel forwarding, `data-lenis-prevent`) diganti alur dokumen biasa dengan sidebar sticky. Tab, kategori, subgrup, dan tag hidup di URL (`?tab=`, `?category=`, `?sub=`, `?tag=`) lewat `pushState` dengan cermin `location.search` plus listener `popstate`, jadi tombol back dan deep link bekerja. Tiga template kartu post disatukan menjadi satu snippet `postCard`.
  - *Future proof*: kursor kustom dan `cursor: none` dihapus, pointer bawaan dikembalikan; section hanya animasi masuk sekali (`once: true`) dan tidak lagi disembunyikan saat scroll balik; `:focus-visible` global bergaya reticle aksen ditambahkan dan penekanan `outline-none`/`ring-0` di kartu Portfolio dibuang; `html, body`, `::selection`, dan scrollbar mengikuti token tema, bukan hitam hardcoded; penolakan promise View Transition saat tab tersembunyi kini ditangkap.
  - *SEO dan bahasa*: `og:image` dan `og:url` absolut dari `identity.url` (baru, `https://suryatmaja.dev`), tag Open Graph dan Twitter per post di reader, `static/robots.txt`, `sitemap.xml` yang di-prerender dari `site.js` dan `src/posts`, serta `hooks.server.js` yang menstempel `lang="id"` untuk semua rute `/blog`.

- **[00:29 WIB] Kartu Sosial Baru dan Dokumentasi Menyeluruh (`static/og-preview.png`, `static/assets/img/global/social-preview.png`, `README.md`, `AI_GUIDELINES.md`, `CONTENT_GUIDE.md`, `CLAUDE.md`):**
  - Kartu sosial 1200x630 digambar ulang mengikuti hero saat ini: nama dalam Epilogue Black, peran dalam Fira Code, hitam dan krem, reticle sudut aksen, figur rasi dengan satu bintang aksen, hairline, dan domain di footer. Dirender lewat System.Drawing dengan font asli situs. Versi lama berjudul "Ngoprek Aja" (peninggalan Jekyll) dan `og-preview.png` beresolusi 1376x768 berfont serif diganti keduanya.
  - `README.md` dan `AI_GUIDELINES.md` ditulis ulang agar sesuai kode sekarang: tiga font, token warna, batas 11px, kosakata motion, `gsap.context` dan `matchMedia`, loader blog server-only, state URL di blog, aturan telemetri asli, daftar anti-pola, verifikasi lewat `preview-build`, dan deployment yang merujuk `Dockerfile`, `nginx.conf`, `docker-compose.yml`, serta workflow GitHub Pages yang benar-benar ada di repo. Referensi ke kursor kustom, `LeftEdgeReturn`, `panel/services.js`, launcher homelab, Pod 042, sinkronisasi `count` di `Skills.svelte`, dan field `cover`/`[!NOTE]` yang tidak pernah didukung dihapus.
  - `CONTENT_GUIDE.md` baru: peta setiap section landing page ke export di `site.js`, skema proyek dan front matter artikel sesuai parser, aturan gambar, cara kerja tab blog, alur manual (`dev`, `build`, changelog, commit), dan contoh prompt untuk AI agent per bagian beserta isi laporan yang diharapkan. `CLAUDE.md` mendapat pointer ke kedua dokumen itu plus kewajiban build dan changelog.
...
### Changed
- **[23:30 WIB] Fondasi Sistem Animasi dan Bundle Client (`lib/motion.js`, `lib/scroll/*`, `lib/components/Hero.svelte`, `routes/(site)/blog/*`, `.claude/launch.json`):**
  - *Bundle halaman utama*: `HeroCanvas` (Threlte + Three.js) kini di-`import()` secara dinamis di `Hero.svelte` hanya setelah `detectTier()` memutuskan tier `full` atau `lite`, jadi klaim "Three.js baru diunduh setelah tiering" di `engineTrivia` akhirnya sesuai kode. Node halaman utama turun dari 833 kB (gzip 221 kB) menjadi 80 kB; scene 3D menjadi chunk terpisah 731 kB yang tidak pernah diminta oleh tier `static`.
  - *Bundle blog*: loader `blog/+page.js` dan `blog/[slug]/+page.js` diganti menjadi `+page.server.js`. Seluruh korpus markdown, Prism, dan js-yaml (chunk 926 kB, gzip 301 kB) tidak lagi dikirim ke browser. Data halaman diambil dari `__data.json` yang ikut di-prerender (1 untuk indeks, 135 untuk post) dan sudah dilayani oleh `try_files $uri` di `nginx.conf`.
  - *Kosakata motion dipaksa*: `sectionAnim.js`, halaman projects, blog index, blog reader, `ResumeModal`, dan `SideNav` tidak lagi menulis durasi dan easing secara literal; semuanya memakai `dur`/`ease`/`stagger` dari `motion.js`. Ditambahkan `stagger.slow` (0.22) untuk blok besar yang datang berurutan, dan `media` berisi query reduced-motion plus breakpoint yang dipakai bersama. `IntroCalibration` sengaja dibiarkan karena ease ratchet, elastic, dan step-nya adalah koreografi khusus, bukan gerak UI.
  - *gsap.context dan gsap.matchMedia*: `sectionAnim` dibangun di dalam `gsap.matchMedia()` berkondisi reduced-motion, jadi mengubah preferensi di tingkat OS langsung membalik tween dan ScrollTrigger tanpa reload. `heroTransition` memakai `matchMedia` untuk breakpoint mobile/tablet/desktop sehingga jarak pin dan lag scrub dibangun ulang saat melewati breakpoint, bukan dibekukan dari nilai saat mount. `SideNav`, `ResumeModal`, halaman projects, blog index, dan blog reader mendaftarkan tween-nya ke `gsap.context` yang di-revert saat unmount.
  - *Satu sumber reduced-motion*: sepuluh cek `window.matchMedia('(prefers-reduced-motion: reduce)')` yang tersebar di komponen dan halaman diganti `prefersReducedMotion()` dari `utils/device.js`.
  - `ResumeModal.close()` sebelumnya memanggil `onClose` dua kali (dari `onComplete` dan `setTimeout`); kini dijaga agar sekali saja. Animasi awal grid di halaman projects yang berjalan dobel (di `onMount` dan di `$effect`) disatukan ke `$effect`.
  - `.claude/launch.json` mendapat konfigurasi `preview-build` (port 4173) untuk memverifikasi hasil `npm run build` tanpa bentrok dengan dev server di 5173.

### Removed
- **[23:30 WIB] Kode Mati (`sectionAnim.js`, `heroTransition.js`, `navigationState.js`, `smoothScroll.js`, `app.css`, `projects/[slug]/+page.svelte`, `lib/components/*`):**
  - Jalur `data-anim-word`, `data-anim-title`, dan aksi `cardHover` di `sectionAnim.js` yang tidak pernah dipakai komponen mana pun; parameter `nameEl` di `heroTransition` beserta `bind:this={nameWrap}` di `Hero.svelte`; tiga fungsi no-op di `navigationState.js`; global debug `window.__st` dan `window.__gsap`; blok CSS `.chromatic-glitch`; atribut `data-project-anim` serta import `gsap` dan `motion` yang tidak terpakai di halaman detail project.
  - Lima komponen yang tidak diimpor di mana pun: `IntroSequence.svelte`, `intro/IntroTacticalCalibration.svelte`, `intro/IntroConstellation.svelte`, `LeftEdgeReturn.svelte`, `AudioFeedback.svelte`, plus `static/assets/img/global/avatar.png` (4,5 MB, tidak direferensikan). Isinya tetap dapat dipulihkan dari commit `6224aac`.
