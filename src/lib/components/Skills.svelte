...
{#snippet spec(tech)}
  <div class="flex flex-col gap-3">
    <div class="flex items-baseline justify-between gap-3 border-b pb-2" style="border-color: var(--tactical-border);">
      <div>
        <span class="block font-mono text-label uppercase tracking-widest" style="color: var(--tactical-text-muted);">Selected</span>
        <h4 class="flex items-center gap-2 text-lg font-semibold tracking-tight">
          <span>{tech.name}</span>
          <span class="font-mono text-label font-normal" style="color: var(--tactical-accent);">{tech.id}</span>
        </h4>
      </div>
      <div class="text-right">
        <span class="block font-mono text-label uppercase tracking-widest" style="color: var(--tactical-text-muted);">Level</span>
        <span class="font-mono text-label uppercase">{tech.badge}</span>
      </div>
    </div>

    <div class="flex flex-col gap-1.5 border-t pt-2.5" style="border-color: var(--tactical-border);">
      <div class="flex items-center justify-between font-mono text-label">
        <span class="uppercase tracking-wider" style="color: var(--tactical-text-muted);">Operational Scope</span>
        <span class="font-medium uppercase tracking-wider px-1.5 py-0.2 border" style="border-color: var(--tactical-accent-border); background-color: var(--tactical-accent-subtle); color: var(--tactical-accent);">
    ...
    <div class="flex flex-col gap-1 border-t pt-2.5" style="border-color: var(--tactical-border);">
      <span class="font-mono text-label uppercase tracking-wider" style="color: var(--tactical-text-muted);">Role</span>
      <p class="border-l pl-2.5 text-caption leading-relaxed" style="border-color: var(--tactical-accent-border); color: var(--tactical-text-primary);">
        {tech.role}
      </p>
    </div>

    <div class="flex flex-col gap-1 border-t pt-2.5" style="border-color: var(--tactical-border);">
      <span class="font-mono text-label uppercase tracking-wider" style="color: var(--tactical-text-muted);">Used in</span>
      <p class="pl-2.5 text-caption leading-relaxed" style="color: var(--tactical-text-muted);">
        {tech.deployedAt}
      </p>
    </div>
  </div>
    ...
    <div class="grid grid-cols-1 items-start gap-6 md:grid-cols-12 lg:gap-8">
          ...
          <div data-anim data-skill-layer class="flex flex-col gap-2.5">
            <div class="flex w-full items-center justify-between border-b border-current/10 pb-2 font-mono text-label uppercase tracking-[0.2em]">
              <div class="flex items-center gap-2">
                <span style="color: var(--tactical-text-muted);">{layer.code || String(li + 1).padStart(2, '0')}</span>
                <span class="font-medium">{layer.layer}</span>
              </div>
              <span style="color: var(--tactical-text-muted);">{layer.items.length} {layer.items.length === 1 ? 'item' : 'items'}</span>
            </div>
            ...
            <div class="flex flex-wrap gap-2 py-1 font-mono">
              {#each layer.items as it (it.id)}
                ...
                <button
                  ...
                  style="background-color: {isCurrent ? 'var(--tactical-surface-elevated)' : 'var(--tactical-surface)'};"
                >
                  <span class="flex h-3.5 w-3.5 shrink-0 items-center justify-center" aria-hidden="true">
                    {#if isCurrent}
                      <span class="h-1.5 w-1.5 rounded-full" style="background-color: var(--tactical-accent);"></span>
                    {:else}
                      <span class="text-label leading-none opacity-40">›</span>
                    {/if}
                  </span>
                  <span class="truncate" style="color: {isCurrent ? 'var(--tactical-accent)' : 'inherit'};">{it.name}</span>
      ...
      <div data-anim class="hidden md:sticky md:top-24 md:col-span-5 md:block">
        <div
          ...
          style="background-color: var(--tactical-surface-elevated);"
        >
          ...
          <div class="flex items-center justify-between border-b border-current/10 pb-3">
            <div class="flex items-center gap-2">
              <span
                ...
                style="background-color: {activeTech ? 'var(--tactical-accent)' : 'var(--tactical-text-muted)'};"
              ></span>
              <span class="text-label font-bold uppercase tracking-wider">Stack inspector</span>
            </div>
            <span class="text-label uppercase tracking-wider" style="color: var(--tactical-text-muted);">
          ...
          {:else}
            <div in:fade={{ duration: reduce ? 0 : 150 }} class="my-1 flex flex-1 flex-col items-center justify-center gap-3 border border-dashed border-current/15 px-4 py-12 text-center">
              ...
              <p class="mx-auto max-w-[26ch] text-caption leading-relaxed" style="color: var(--tactical-text-muted);">
          ...
          <!-- Real counts only -->
          <div class="flex flex-wrap items-center justify-between gap-2 border-t border-current/10 pt-3 text-label" style="color: var(--tactical-text-muted);">
    ...
    {#if interacted && activeTech}
      <div class="sticky bottom-4 z-30 pt-1 md:hidden">
        <button
          ...
          class="tactical-invert-hover flex w-full cursor-pointer items-center justify-between border px-3.5 py-2.5 font-mono text-xs backdrop-blur-md transition-all"
          style="background-color: var(--tactical-surface-elevated); border-color: var(--tactical-border); color: var(--tactical-text-primary);"
        >
          <div class="flex items-center gap-2">
            <span class="h-2 w-2 rounded-full" style="background-color: var(--tactical-accent);"></span>
            <span class="font-semibold tracking-wider">{activeTech.name}</span>
            <span class="text-label border px-1 py-0.5 uppercase tracking-wider" style="border-color: var(--tactical-border); color: var(--tactical-accent);">{activeTech.badge}</span>
          </div>
          <span class="border px-1.5 py-0.5 text-label uppercase tracking-wider" style="border-color: var(--tactical-border);">Details ↗</span>
        </button>
      </div>
    {/if}
  </div>
...
{#if showMobileInspector && (activeTech || allTools[0])}
  ...
  >
    <div
      ...
      data-lenis-prevent
    >
      <span class="pointer-events-none absolute -top-px -left-px h-2.5 w-2.5 border-l-2 border-t-2" style="border-color: var(--tactical-accent);" aria-hidden="true"></span>
      <span class="pointer-events-none absolute -top-px -right-px h-2.5 w-2.5 border-r-2 border-t-2" style="border-color: var(--tactical-accent);" aria-hidden="true"></span>

      <div class="flex items-center justify-between border-b pb-3" style="border-color: var(--tactical-border);">
        <div class="flex items-center gap-2">
          <span class="h-2 w-2 rounded-full" style="background-color: var(--tactical-accent);"></span>
          <span class="text-label font-bold uppercase tracking-wider">Stack inspector</span>
        </div>
        <button
          type="button"
          onclick={() => (showMobileInspector = false)}
          class="tactical-invert-hover cursor-pointer border px-2 py-1 font-mono text-label uppercase tracking-wider transition-colors"
      ...
      <button
        type="button"
        onclick={() => (showMobileInspector = false)}
        class="tactical-invert-hover mt-2 w-full cursor-pointer border py-2.5 text-center font-mono text-xs uppercase tracking-wider transition-colors"
        style="border-color: var(--tactical-border); background-color: var(--tactical-surface); color: var(--tactical-text-primary);"
      >
        Back to the list
      </button>
